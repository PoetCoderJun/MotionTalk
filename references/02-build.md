...
临时入口至少支持 `preview`、`segment`、`package` 三种 profile；正式制作只运行 `segment`。它属于项目中间产物，不能写回或复制到 Skill 仓库。
...
只有两类情况才按需运行临时批量入口的 `preview <cue-id...>` profile：用户明确要求动态预览，或某个 cue 的语义顺序、节奏或视频层同步无法靠关键帧判断。不得默认预览全部 cue。
