...
```mermaid
flowchart LR
    A["给 AI 视频和 SRT"] --> B["说几句想要的效果<br/>或点名参考主题"]
    B --> C["AI 看完内容<br/>给出导演方案"]
    C --> D["你确认一次"]
    D --> E["AI 连续制作与检查<br/>交付最终成片"]
```

例如只需要说：

```text
用 MotionTalk 处理这条视频和对应字幕。做成竖屏，PPT 居中，人物放右下角，
字幕最多两行；整体简洁一点。先给我看导演方案。
```

方案确认后，AI 会连续制作、检查并交付最终成片，不需要反复回复“继续”。
